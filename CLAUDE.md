...
| Archivo | Propósito |
|---|---|
| `src/hooks/index.js` | Cuatro hooks de React: `useAuth()`, `useClientes()`, `useCotizaciones(clienteId?)`, `useTarifas()`. `useAuth()` ahora también carga el `rol` del usuario desde `public.users` y lo expone como `{ user, rol, loading }`. |
